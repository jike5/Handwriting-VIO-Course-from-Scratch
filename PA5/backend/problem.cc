...
void Problem::MakeHessian() {
    ...
    for (auto &edge: edges_) {
        ...
        assert(jacobians.size() == verticies.size()); // Verticies_(include address of vertex class)and Jacobians_(contain MatXX) is vector,
        //         for example the EdgeReprojection class is has three verties so the size is 3
